...
camera's intrinsics. (The conversion from 11BIT->REGISTERED is done in software
by libfreenect, not in Kinect hardware.)

Before this custom fork, fakenect only supported 11BIT mode.  This custom fork
allows the depth images to be recorded in 11BIT mode as normal, but then played
back in either 11BIT or REGISTERED mode.
