...
// (cvk) dumps the kinect's registration data to a file called `registration`
// in the `out_dir` directory. later, fakenect can use this registration data
// to turn recorded raw 11BIT depth frames into REGISTERED depth frames (which
// have been registered to the intrinsics of the RGB camera).
//
...
void dump_registration(freenect_device *f_dev) {
	char registration_filename[512];
	sprintf(registration_filename, "%s/registration", out_dir); // buffer overflow, lol
...
void init()
{
	...
	// (cvk) save the kinect's registration data to a file so it can be
	// used for playback of this recording
	dump_registration(dev);
